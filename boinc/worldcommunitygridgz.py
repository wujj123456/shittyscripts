...
PROPERTIES = ("GrantedCredit", "ElapsedTime", "DeviceName", "ReceivedTime", "AppName")


def base_stat_url(username, validation_key_file):
    STAT_URL = (
        "https://secure.worldcommunitygrid.org/api/members/{user}/"
        "results?code={key}&ValidateState=1"
    )
    with open(os.path.expanduser(validation_key_file), "r") as f:
        key = f.readlines()[0].strip()
    return STAT_URL.format(user=username, key=key)
...
def get_web_data(username, password_file, validation_key_file):
    ...
    try:
        ...
        while True:
            ...
            print(
                "Fetch from offset={}. Returned {} results".format(
                    offset, new_data["ResultsReturned"]
                )
            )
...
def update_data_from_web(data, username, password_file, validation_key_file):
    web_data = get_web_data(username, password_file, validation_key_file)
    if not web_data:
        return data

    for app, r in web_data.items():
        if data[app]:
            # verify all information match for debugging
...
def save_data_to_disk(data, data_file):
...
def parse_args(args):
    ...
    parser.add_argument(
        "--password-file",
        default="~/.wcg.password",
        help="Location to file containing account key",
    )
    parser.add_argument(
        "--validation-key-file",
        default="~/.wcg.key",
        help="Location to file containing validation key for stats",
    )
    parser.add_argument(
        "--data-file",
        default="worldcommunitygrid.txt.gz",
        help="JSON file containing historical data to be updated",
    )
    parser.add_argument("--username", default="wujj123456", help="WCG username")
...
def main():
    ...
    data = get_disk_data(args.data_file)
    if not data:
        data = defaultdict(dict)
    if not args.analysis_only:
        data = update_data_from_web(
            data, args.username, args.password_file, args.validation_key_file
        )
        save_data_to_disk(data, args.data_file)
    if args.update_only:
        return
